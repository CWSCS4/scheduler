 ...
 * system requires the following data files to create a full database:
 * - List of student's classes ( studentName, className, teacherName )
 * - List of student's advisors ( studentName, advisorName )
 * - List of birthdays for creating passwords ( studentName, dateOfBirth )
 * - List of conference rooms ( teacherName, room )
 *    * "room" should contain the word or numeral for the floor or "basement"
 *
 * Run this program passing it the name of each file. The database entries
 * for each type of data will need to be cleared from the database before 
 * the new data is entered. The database is assumed to be running on the local
 * computer and the database is called "scheduler" and the user "scheduler"
 * has complete permissions to change the database.
 * 
 * Syntax:
...
public class LoadData {
	...
	try {
	    ...
	    connect = DriverManager.getConnection( "jdbc:mysql://" + hostname + "/" + database, username, dbpass );
	...
	// Create the tables.
	System.out.println( "Initializing a blank database has not been implemented yet." );
    }

    public void erase() {
	// Remove all data from tables. Leave perminant configuration.
	try {
	...
	try {
	    PreparedStatement queryStudent = connect.prepareStatement( "SELECT studentID, name FROM students WHERE name = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
	    PreparedStatement queryTeacher = connect.prepareStatement( "SELECT teacherID, name FROM teachers WHERE name = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
	    PreparedStatement queryClass = connect.prepareStatement( "SELECT classID, name, teacherID FROM classes WHERE name = ? AND teacherID = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
	    PreparedStatement queryMembers = connect.prepareStatement( "SELECT classID, studentID FROM classMembers WHERE classID = ? AND studentID = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
		...
		if ( results.first() ) {
		    System.out.println( "Duplicate entry! (" + name + "," + line[1] + "," + tname + ") Ignoring." );
		} else {
		    results.moveToInsertRow();
	...
	try {
		...
		results = queryMembers.executeQuery();
		if ( results.first() ) {
		    System.out.println( "Duplicate entry! (" + line[0] + ",Advisor," + line[1] + ") Ignoring." );
		} else {
		    results.moveToInsertRow();
		    results.updateInt( 1, classID );
		    results.updateInt( 2, studentID );
	...
	try {
	    ...
	    PreparedStatement queryStudent = connect.prepareStatement( "SELECT studentID, name, password FROM students WHERE name = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
	...
	try {
	    PreparedStatement queryTeacher = connect.prepareStatement( "SELECT teacherID, name, room FROM teachers WHERE name = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
	...
	try {
	    ...
	    PreparedStatement queryTeachers = connect.prepareStatement( "SELECT teacherID, password FROM teachers", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE );
	...
	/*int i = name.indexOf( ',' );
	if ( i >= 0 ) {
	    return name.substring( i+2, name.length() ) + " " + name.substring( 0, i );
	}
	return name;*/
	return cleanTeacherName( name ); // Now that we are formatting the names the same way, use the same code.
    }
